...
- [x] At least one valid record, using either `CNAME` or `A` — not both
- [x] On Vercel: both the CNAME and the `vc-domain-verify` TXT record are present, with `"provider": "vercel"`
- [x] The CNAME points at my provider's custom-domain target, not a `*.vercel.app` deployment alias
...
Next: [After Merge](/guides/workflow/after-merge)
