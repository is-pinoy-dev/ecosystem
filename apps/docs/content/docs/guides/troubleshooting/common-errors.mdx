...
**Fix:** Choose a subdomain that is at least 3 characters long. Single or double-character subdomains are not allowed:
...
**Error:**
```text
❌ Reserved subdomain: status
```

**Fix:** Choose a different name. Certain subdomains are reserved for platform use (e.g. `api`, `status`, `mail`, `admin`). See the full list in [Naming Rules](/guides/getting-started/naming-rules#reserved-names).
