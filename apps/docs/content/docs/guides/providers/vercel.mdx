...
- **The TXT record is temporary** — Vercel only checks it once, during verification. After your subdomain is verified and live, the registry may automatically remove the TXT record from your JSON file (via a cleanup PR). This is safe and expected — if you ever move the subdomain to a different Vercel account, Vercel issues a fresh verification value anyway.
