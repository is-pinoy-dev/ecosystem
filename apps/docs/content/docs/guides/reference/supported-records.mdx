...
The `provider` field accepts one of: `vercel`, `netlify`, `github`, `cloudflare`. It determines the DNS record name — a verification TXT is created at `_<provider>.yourname.is-pinoy.dev`.
