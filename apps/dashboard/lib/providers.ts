// How a host is presented in the dashboard: its mark, and whether a record
// pointed at it may be proxied.
//
// Recognising the host from a CNAME target is not done here — that fingerprint
// is shared with the public showcase and lives in `@is-pinoy-dev/validate`
// (src/host.ts). This module adds only what the dashboard needs on top of it,
// so the two surfaces can never disagree about what a subdomain points at.
//
...
export const PROVIDERS = {
  vercel: {
    id: "vercel",
    name: HOST_PROVIDER_NAMES.vercel,
  ...
  github: {
    id: "github",
    name: HOST_PROVIDER_NAMES.github,
  ...
  netlify: {
    id: "netlify",
    name: HOST_PROVIDER_NAMES.netlify,
  ...
  cloudflare: {
    id: "cloudflare",
    name: HOST_PROVIDER_NAMES.cloudflare,
  ...
  portfolio: {
    id: "portfolio",
    name: HOST_PROVIDER_NAMES.portfolio,
...
} as const satisfies Record<HostProviderId, Provider>

export type ProviderId = HostProviderId
...
/** The presentation of the host a CNAME target points at, or null if unknown. */
export function providerForTarget(target: string): Provider | null {
  const id = hostProviderForTarget(target)
  return id ? PROVIDERS[id] : null
}

/** The presentation of the host a whole subdomain is served by, or null. */
export function providerForRecords(
  records: Record<string, unknown>
): Provider | null {
  const id = hostProviderForRecords(records)
  return id ? PROVIDERS[id] : null
}
