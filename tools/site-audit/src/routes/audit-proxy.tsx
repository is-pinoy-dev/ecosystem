...
import { upstreamFor, type PortfolioEnv } from "../lib/upstream";
import type { WaitUntil } from "../lib/domains";
...
export async function loader({ request, context }: Route.LoaderArgs) {
  ...
  try {
    const cf = (
      context as {
        cloudflare?: { env?: PortfolioEnv; ctx?: WaitUntil };
      }
    )?.cloudflare;
    const upstream = await upstreamFor(
      parsedTarget,
      (cf?.env ?? {}) as PortfolioEnv,
      cf?.ctx,
    );
